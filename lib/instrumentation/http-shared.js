    ...
    return function (event, req, res) {
      if (event === 'request') {
        ...
        if (isRequestBlacklisted(agent, req)) {
          ...
          var traceparent = req.headers['zuqa-apm-traceparent'] || req.headers.traceparent
  ...
  return function (orig) {
    return function (...args) {
      ...
      if (parent && parent._context && shouldPropagateTraceContext(options)) {
        ...
        if (agent._conf.useZuqaTraceparentHeader) {
          options.headers['zuqa-apm-traceparent'] = headerValue
        }
      }
