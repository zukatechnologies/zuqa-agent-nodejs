    ...
    const next = afterAll(() => {
      ...
      if (this._agent.collectActively) {
        for (const metric of seen.values()) {
          if (Object.keys(metric.samples).length > 3) {
            this.metricArray.push(metric)
          }
        }
      }

      if (!this._agent.collectActively && this.metricArray.length > 0) {
        this._agent.logger.debug('sending transaction_metricset %o', { url: this._agent.lastTransactionName, metricsetsSize: this.metricArray.length })
