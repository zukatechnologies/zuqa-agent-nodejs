...
Agent.prototype._config = function (opts) {
  ...
  const { host, port } = this._conf.serverUrl
    ...
    : { host: 'localhost:32140', port: '32140' }
...
Agent.prototype.start = function (opts) {
  ...
  if (!this._conf.active) {
    this.logger.debug('Zuqa APM agent disabled (`active` is false)')
    return this
  } else if (!this._conf.serviceName) {
    this.logger.error('Zuqa APM isn\'t correctly configured: Missing serviceName')
    this._conf.active = false
    return this
  } else if (!/^[a-zA-Z0-9 _-]+$/.test(this._conf.serviceName)) {
    this.logger.error('Zuqa APM isn\'t correctly configured: serviceName "%s" contains invalid characters! (allowed: a-z, A-Z, 0-9, _, -, <space>)', this._conf.serviceName)
    this._conf.active = false
    return this
  } else if (this._conf.serverPort < 1 || this._conf.serverPort > 65535) {
    this.logger.error('Zuqa APM isn\'t correctly configured: serverUrl "%s" contains an invalid port! (allowed: 1-65535)', this._conf.serverUrl)
    this._conf.active = false
    return this
...
Agent.prototype.captureError = function (err, opts, cb) {
  ...
  function send (error) {
    ...
    if (agent._transport) {
      agent.logger.info('Sending error to Zuqa APM', { id })
...
Agent.prototype.handleUncaughtExceptions = function (cb) {
  ...
  this._uncaughtExceptionListener = function (err) {
    ...
    agent.logger.debug('Zuqa APM caught unhandled exception: %s', err.message)
