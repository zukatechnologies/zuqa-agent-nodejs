...
var ZuqaAPMHttpClient = require('../zuqa-apm-http-client')
var readPkgUp = require('read-pkg-up')
var truncate = require('unicode-byte-truncate')

var version = require('../package').version
var packageName = require('../package').name
// Standardize user-agent header. Only use "zuqaagent-node" if it matches "zuqa-agent-node".
if (packageName === 'zuqa-agent-node') {
  packageName = 'zuqaagent-node'
}
...
var DEFAULTS = {
  ...
  useZuqaTraceparentHeader: true,
  usePathAsTransactionName: false,
  verifyServerCert: true
}

var ENV_TABLE = {
  abortedErrorThreshold: 'ZUQA_APM_ABORTED_ERROR_THRESHOLD',
  active: 'ZUQA_APM_ACTIVE',
  addPatch: 'ZUQA_APM_ADD_PATCH',
  apiRequestSize: 'ZUQA_APM_API_REQUEST_SIZE',
  apiRequestTime: 'ZUQA_APM_API_REQUEST_TIME',
  asyncHooks: 'ZUQA_APM_ASYNC_HOOKS',
  breakdownMetrics: 'ZUQA_APM_BREAKDOWN_METRICS',
  captureBody: 'ZUQA_APM_CAPTURE_BODY',
  captureErrorLogStackTraces: 'ZUQA_APM_CAPTURE_ERROR_LOG_STACK_TRACES',
  captureExceptions: 'ZUQA_APM_CAPTURE_EXCEPTIONS',
  captureHeaders: 'ZUQA_APM_CAPTURE_HEADERS',
  captureSpanStackTraces: 'ZUQA_APM_CAPTURE_SPAN_STACK_TRACES',
  centralConfig: 'ZUQA_APM_CENTRAL_CONFIG',
  containerId: 'ZUQA_APM_CONTAINER_ID',
  disableInstrumentations: 'ZUQA_APM_DISABLE_INSTRUMENTATIONS',
  environment: 'ZUQA_APM_ENVIRONMENT',
  errorMessageMaxLength: 'ZUQA_APM_ERROR_MESSAGE_MAX_LENGTH',
  errorOnAbortedRequests: 'ZUQA_APM_ERROR_ON_ABORTED_REQUESTS',
  filterHttpHeaders: 'ZUQA_APM_FILTER_HTTP_HEADERS',
  frameworkName: 'ZUQA_APM_FRAMEWORK_NAME',
  frameworkVersion: 'ZUQA_APM_FRAMEWORK_VERSION',
  globalLabels: 'ZUQA_APM_GLOBAL_LABELS',
  hostname: 'ZUQA_APM_HOSTNAME',
  instrument: 'ZUQA_APM_INSTRUMENT',
  instrumentIncomingHTTPRequests: 'ZUQA_APM_INSTRUMENT_INCOMING_HTTP_REQUESTS',
  kubernetesNamespace: ['ZUQA_APM_KUBERNETES_NAMESPACE', 'KUBERNETES_NAMESPACE'],
  kubernetesNodeName: ['ZUQA_APM_KUBERNETES_NODE_NAME', 'KUBERNETES_NODE_NAME'],
  kubernetesPodName: ['ZUQA_APM_KUBERNETES_POD_NAME', 'KUBERNETES_POD_NAME'],
  kubernetesPodUID: ['ZUQA_APM_KUBERNETES_POD_UID', 'KUBERNETES_POD_UID'],
  logLevel: 'ZUQA_APM_LOG_LEVEL',
  logUncaughtExceptions: 'ZUQA_APM_LOG_UNCAUGHT_EXCEPTIONS',
  metricsInterval: 'ZUQA_APM_METRICS_INTERVAL',
  metricsLimit: 'ZUQA_APM_METRICS_LIMIT',
  payloadLogFile: 'ZUQA_APM_PAYLOAD_LOG_FILE',
  serverCaCertFile: 'ZUQA_APM_SERVER_CA_CERT_FILE',
  secretToken: 'ZUQA_APM_SECRET_TOKEN',
  serverTimeout: 'ZUQA_APM_SERVER_TIMEOUT',
  serverUrl: 'ZUQA_APM_SERVER_URL',
  serviceName: 'ZUQA_APM_SERVICE_NAME',
  serviceNodeName: 'ZUQA_APM_SERVICE_NODE_NAME',
  serviceVersion: 'ZUQA_APM_SERVICE_VERSION',
  sourceLinesErrorAppFrames: 'ZUQA_APM_SOURCE_LINES_ERROR_APP_FRAMES',
  sourceLinesErrorLibraryFrames: 'ZUQA_APM_SOURCE_LINES_ERROR_LIBRARY_FRAMES',
  sourceLinesSpanAppFrames: 'ZUQA_APM_SOURCE_LINES_SPAN_APP_FRAMES',
  sourceLinesSpanLibraryFrames: 'ZUQA_APM_SOURCE_LINES_SPAN_LIBRARY_FRAMES',
  stackTraceLimit: 'ZUQA_APM_STACK_TRACE_LIMIT',
  transactionMaxSpans: 'ZUQA_APM_TRANSACTION_MAX_SPANS',
  transactionSampleRate: 'ZUQA_APM_TRANSACTION_SAMPLE_RATE',
  useZuqaTraceparentHeader: 'ZUQA_APM_USE_ZUQA_TRACEPARENT_HEADER',
  usePathAsTransactionName: 'ZUQA_APM_USE_PATH_AS_TRANSACTION_NAME',
  verifyServerCert: 'ZUQA_APM_VERIFY_SERVER_CERT'
}
  ...
  constructor (opts) {
    ...
    if (typeof this.transport !== 'function') {
      this.transport = function httpTransport (conf, agent) {
        var transport = new ZuqaAPMHttpClient({
          // metadata
          agentName: 'nodejs',
...
function loadConfigFile (configFile) {
  const confPath = path.resolve(configFile || process.env.ZUQA_APM_CONFIG_FILE || 'zuqa-agent-node.js')

  if (fs.existsSync(confPath)) {
    try {
      return require(confPath)
    } catch (err) {
      console.error('Zuqa APM initialization error: Can\'t read config file %s', confPath)
...
function loadServerCaCertFile (opts) {
  if (opts.serverCaCertFile) {
    try {
      ...
      opts.logger.error('Zuqa APM initialization error: Can\'t read server CA cert file %s (%s)', opts.serverCaCertFile, err.message)
    }
  }
}
