...
* *Type:* String
* *Default:* `zuqa-apm-node.js`
* *Env:* `ZUQA_APM_CONFIG_FILE`

The Node.js agent will look for a file named `zuqa-apm-node.js` in the current working directory.
You can specify a custom path using this config option (this path must include the filename), e.g:

[source,bash]
----
ZUQA_APM_CONFIG_FILE=/path/to/my-zuqa-apm-node.js
----
