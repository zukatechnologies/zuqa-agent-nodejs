...
IMPORTANT: The Zuqa APM agent for Node.js needs to be started as the first thing in your application - *before any other module is required/imported*.
...
There are two ways to start the Zuqa APM agent for Node.js:

* Require the `zuqa-agent-node` module and call the <<apm-start,`start`>> function on the returned agent:
+
[source,js]
----
...
----
* Require the `zuqa-agent-node/start` script:
+
[source,js]
----
var apm = require('zuqa-agent-node/start')
...
NOTE: If you are using Babel, you need to use the `zuqa-agent-node/start` approach.
