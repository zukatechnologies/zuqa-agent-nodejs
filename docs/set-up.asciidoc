...
* Require the `zuqa-apm-node` module and call the <<apm-start,`start`>> function on the returned agent:
+
[source,js]
----
...
----
* Require the `zuqa-apm-node/start` script:
+
[source,js]
----
var apm = require('zuqa-apm-node/start')
...
NOTE: If you are using Babel, you need to use the `zuqa-apm-node/start` approach.
