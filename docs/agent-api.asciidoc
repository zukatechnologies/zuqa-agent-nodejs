...
NOTE: This is the API documentation for the Zuqa APM Node.js Agent.
For getting started,
...
The Zuqa APM agent for Node.js is a singleton.
You get the agent instance by either requiring `zuqa-apm-node` or `zuqa-apm-node/start`.
...
[source,js]
----
const apm = require('zuqa-apm-node').start(...)
----

If you need to access the `Agent` in any part of your codebase,
you can simply require `zuqa-apm-node` to access the already started singleton.
You therefore don't need to manage or pass around the started `Agent` yourself.

[[apm-start]]
==== `apm.start([options])`

Starts the Zuqa APM agent for Node.js and returns itself.
...
It is recommended to use `-r zuqa-apm-node/start` when starting the app to avoid this.
====
...
most of which are automatically managed by the Zuqa APM Node.js Agent.
...
// add Zuqa APM in the bottom of the middleware stack
