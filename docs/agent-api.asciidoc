...
You get the agent instance by either requiring `zuqa-agent-node` or `zuqa-agent-node/start`.
...
you can simply require `zuqa-agent-node` to access the already started singleton.
...
It is recommended to use `-r zuqa-agent-node/start` when starting the app to avoid this.
====
