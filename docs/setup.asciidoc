...
Instead you need to import the `zuqa-agent-node/start` module:

[source,js]
----
import apm from 'zuqa-agent-node/start'
----
...
To explicitly start the agent you need to import `zuqa-agent-node` and call the `apm.start()` method with the
...
[source,js]
----
import * as apm from 'zuqa-agent-node/start'
----
