...
Instead you need to import the `zuqa-apm-node/start` module:

[source,js]
----
import apm from 'zuqa-apm-node/start'
----
...
To explicitly start the agent you need to import `zuqa-apm-node` and call the `apm.start()` method with the
...
[source,js]
----
import * as apm from 'zuqa-apm-node/start'
----
